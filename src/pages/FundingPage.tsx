...
const FundingPage: React.FC = () => {
  ...
  const fundingOptions = [
    ...
    },
    {
      ...
      highlights: ["50+ investors", "Direct introductions", "Follow-on funding"],
    },
    {
      icon: Landmark,
  ...
  const processSteps = [
    { icon: FileCheck, label: "Apply", desc: "Submit your startup profile, traction metrics, and funding requirements" },
    { icon: SearchCheck, label: "Review", desc: "Our investment committee evaluates your application and market potential" },
    { icon: PresentationIcon, label: "Pitch", desc: "Present to our panel of investors and mentors in a structured pitch session" },
    { icon: Banknote, label: "Fund", desc: "Receive funding, sign agreements, and get post-investment support" },
  ];

  const metrics = [
    { target: 85, suffix: "L+", prefix: "INR ", label: "Total Funding Raised", icon: IndianRupee },
    ...
    <div className="min-h-screen bg-black overflow-hidden">
      ...
      <motion.section className="relative min-h-screen flex items-center justify-center pt-20" style={{ y: heroY, opacity: heroOpacity }}>
        ...
        {[...Array(18)].map((_, i) => (
          <motion.div
            ...
            animate={{ y: [0, -(200 + Math.random() * 400)], opacity: [0, 0.8, 0], scale: [0, 1, 0] }}
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 40, rotateX: 15 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} transition={{ duration: 1 }} style={{ perspective: 1000 }}>
            ...
            >
              ...
              <span className="text-green-300 text-sm font-medium">Fuel Your Growth</span>
            </motion.div>
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-white leading-none mb-6">
              {content?.heroTitle || (
                <>
                  Funding{" "}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 via-emerald-400 to-purple-400">Support</span>
                </>
              )}
            </h1>
            <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-10">
              {content?.heroSubtitle || "From seed capital to VC introductions, we help promising startups access the funding they need to scale."}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/register" className="px-8 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-bold text-lg hover:from-green-500 hover:to-emerald-500 transition-all duration-300 shadow-lg shadow-green-500/25">
                Apply for Funding
              </Link>
              <a href="#options" className="px-8 py-4 bg-white/5 border border-white/20 text-white rounded-xl font-bold text-lg hover:bg-white/10 transition-all duration-300">
                Explore Options
              </a>
            </div>
          </motion.div>
        ...
        <motion.div className="absolute bottom-10 left-1/2 -translate-x-1/2" animate={{ y: [0, 12, 0] }} transition={{ duration: 2, repeat: Infinity }}>
          <div className="w-6 h-10 border-2 border-white/20 rounded-full flex justify-center pt-2">
            <motion.div className="w-1.5 h-1.5 bg-green-400 rounded-full" animate={{ y: [0, 16, 0] }} transition={{ duration: 2, repeat: Infinity }} />
          </div>
        </motion.div>
      </motion.section>
      ...
      <section id="options" className="relative z-10 py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.h2 className="text-4xl md:text-5xl font-black text-center text-white mb-4" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            Funding{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400">Options</span>
          </motion.h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {fundingOptions.map((opt, i) => (
              <motion.div key={opt.title} initial={{ opacity: 0, y: 30, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}>
                <GlassCard className="p-8 h-full group hover:border-green-500/40 transition-all duration-500 relative overflow-hidden">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {opt.highlights.map((h) => (
                      <span key={h} className="px-3 py-1 bg-green-500/10 border border-green-500/20 rounded-full text-green-400 text-xs font-medium">
                        {h}
                      </span>
                    ))}
                  </div>
      ...
      <section className="relative z-10 py-24 px-6 bg-gradient-to-b from-transparent via-green-900/5 to-transparent">
        <div className="max-w-6xl mx-auto">
          <motion.h2 className="text-4xl md:text-5xl font-black text-center text-white mb-4" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            The{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-purple-400">Process</span>
          </motion.h2>
          <motion.p className="text-gray-400 text-center mb-16 max-w-xl mx-auto" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            ...
            {processSteps.map((step, i) => (
              <motion.div key={step.label} initial={{ opacity: 0, y: 40, rotateY: -15 }} whileInView={{ opacity: 1, y: 0, rotateY: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.6 }}>
                <GlassCard className="p-6 text-center relative group hover:border-green-500/40 transition-all duration-500">
                  ...
                  <motion.div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-green-500/20 to-emerald-500/20 rounded-2xl flex items-center justify-center border border-green-500/20" whileHover={{ scale: 1.1, rotateZ: 5 }}>
                    <step.icon className="w-8 h-8 text-green-400" />
                  </motion.div>
                  <h3 className="text-xl font-bold text-white mb-2">{step.label}</h3>
                  <p className="text-gray-400 text-sm">{step.desc}</p>
                  {i < 3 && <ChevronRight className="hidden md:block absolute -right-5 top-1/2 -translate-y-1/2 w-6 h-6 text-green-500/50" />}
                </GlassCard>
              </motion.div>
            ))}
          </div>
      ...
      <section className="relative z-10 py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <motion.h2 className="text-4xl md:text-5xl font-black text-center text-white mb-4" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            Our{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-400">Impact</span>
          </motion.h2>
          <motion.p className="text-gray-400 text-center mb-16 max-w-xl mx-auto" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            <GlassCard className="p-12 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <h2 className="text-3xl md:text-4xl font-black text-white mb-4">Ready to Get Funded?</h2>
                <p className="text-gray-400 mb-8 max-w-lg mx-auto">Apply now and let us connect you with the right funding opportunities for your startup's stage and vision.</p>
                <Link to="/register" className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-bold text-lg hover:from-green-500 hover:to-emerald-500 transition-all duration-300 shadow-lg shadow-green-500/25">
